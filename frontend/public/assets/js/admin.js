...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('http://localhost:4455/api/admin/login', {
                    method: 'POST',
                    headers: {
...
function fetchProducts() {
    ...
    fetch('http://localhost:4455/api/admin/products', { 
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('authToken')}`
...
async function updateProductStatus(productId, isActive, toggleElement) {
    try {
        const response = await fetch(`http://localhost:4455/api/admin/products/${productId}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('authToken')}`
...
function handleDeleteProduct(event) {
    ...
    if (confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบสินค้า ID: ${productId}?`)) {
        ...
        fetch(`http://localhost:4455/api/products/${productId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('authToken')}`
...
async function saveProduct() {
    ...
    const url = productId 
        ? `http://localhost:4455/api/products/${productId}` // URL for editing
        : 'http://localhost:4455/api/products';            // URL for adding
...
function fetchContacts(filter = 'all') {
    ...
    fetch(`http://localhost:4455/api/contact?${queryParams}`, {
        headers: {
...
function viewContactDetails(contactId) {
    ...
    fetch(`http://localhost:4455/api/contact/${contactId}`, {
        headers: {
            'Authorization': `Bearer ${authToken}`
        }
    })
...
function markContactAsRead(contactId) {
    ...
    fetch(`http://localhost:4455/api/contact/${contactId}/read`, {
        method: 'PUT',
        headers: {
            'Authorization': `Bearer ${authToken}`,
            'Content-Type': 'application/json'
...
function loadPaymentSettings() {
    ...
    fetch('http://localhost:4455/api/payment/settings', {
        headers: {
            'Authorization': `Bearer ${authToken}`
        }
    })
...
function generatePromptpayQR() {
    ...
    fetch(`http://localhost:4455/api/payment/promptpay/qr?amount=${amount}`)
    .then(response => response.json())
    .then(data => {
...
async function sendWebhookTest() {
    ...
    const webhookUrl = document.getElementById('webhookUrl')?.value || 'http://localhost:4455/api/payment/verify';
